...
// Fragment stage for the abyss effect: a depth of something seen through the item's outline.
//
// 🔴 A COPY of cosmic.fsh, deliberately kept separate. This is the file to rewrite as the abyss gets its own
// look. Do NOT merge it back with cosmic.fsh — that program's appearance has been accepted frame by frame,
// and keeping the two apart is what lets this one change freely.
//
...
void main() {
	...
	// against it means the value here is the drift over a whole day — that is what ABYSS_DRIFT_SPEED states,
	...
	float drift = GameTime * ABYSS_DRIFT_SPEED;

	for (int i = 0; i < int(ABYSS_LAYERS); i++) {
		int mult = int(ABYSS_LAYERS) - i;
		...
		// ABYSS_DENSITY: 1.0 is the reference exactly, 2.0 doubles how many cells hold a star.
		int symbol = int(rand2d(vec2(float(tu), float(tv) + float(i) * 10.0)) * CELL_OUT_OF);

		if (symbol >= 0 && float(symbol) < float(SPRITE_COUNT) * ABYSS_DENSITY) {
			// 🔴 Which sprite this cell draws. Taken modulo SPRITE_COUNT because the test above admits
			// indices up to SPRITE_COUNT * ABYSS_DENSITY, and ABYSS_DENSITY may exceed 1.0 — a density of
	...
	//
	// Distinct from ABYSS_BRIGHTNESS, which is also a multiplier: that one is a compile-time define and part of
