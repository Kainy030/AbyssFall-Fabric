...
/**
 * Hands an attack made with the Final Death Omen to the blade's own resolution, and leaves
 * every other attack in the game exactly as it found it.
 *
 * <p>The trigger is twofold: the blade itself by identity first of all, and the
 * {@code abyssfall:abyss_striking} tag as the second gate, through which a modpack
 * author's weapon joins the verdict. What the tag does <em>not</em> carry is the Omen's
 * voice — kills by non-Omen members read the configurable generic text
 * ({@code striking.death_message}); see {@code AbyssStrikeDamageSource}.
 *
	...
	private void abyssfall$resolveDeathOmen(Entity target, Operation<Void> original) {
		...
		if (!(self.level() instanceof ServerLevel level)
				|| !(self.getWeaponItem().is(AbyssFallItems.FINAL_DEATH_OMEN)
						|| self.getWeaponItem().is(AbyssFallItemTags.ABYSS_STRIKING))) {
