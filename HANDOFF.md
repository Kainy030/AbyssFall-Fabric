...
> | **`REFERENCE.md`** | 已实现的 16 个功能逐个的实现细节与设计依据、Git/发布流程、CI、gh 用法 | **按需查**。要动某个已有功能之前，先读它那一节 |
>
...
**四个 tag 的 Release workflow 全部 `completed success`**：`0.1-Dev`、`v0.2-Dev`、`v0.3-Dev`、`v0.4-Dev`，耗时 1m25s ~ 1m49s；`v0.4-Dev` 的 Release 资产实测含三个 jar。这推翻了前几次交接「推送 tag 后没等 CI、无法查询」的记载。

**`gh` CLI 现在装好了，以后自己查，别再留成未验证项。** 用法与那个 token scope 的坑见 `REFERENCE.md` 的「gh CLI」一节。
...
**配置系统（已可用，可继续扩展）**
- 目前 **4 个块、7 个配置项**（`developer` 2 + `loot` 2 + `visuals` 2 + `hud` 1）。用户预期「以后可自定义配置会特别多」，架构已就绪，加块加项照「配置系统」一节的流程即可
