...
> 🛑 **这个分支（`1.21.11`）已停止维护。**
>
> 开发已全部转移到 `main` 分支，目标版本 **Minecraft 26.2**。本文件描述的是 1.21.11
> 时期的项目状态，**刻意冻结、不再更新**。若你是来接手开发的，请去 `main` 分支读那边的
> `HANDOFF.md`——那里的架构约定、血泪教训与当前状态才是有效的。
>
> 本文件仍然有价值：26.2 的设计决策绝大多数是从这里延续下来的，遇到「为什么当初这么做」
> 的疑问可以回来查。但**不要**按这里的版本号、API 名或构建方式去写 26.2 的代码。
...
- 🔴 **不要动那些看起来「不够优雅」的代码。** 用户第七次交接原话：「项目中所有看起来'不够优雅'的代码全部都是你或者你曾经经过验证后不得不这么做的妥协之法，不要瞎鸡巴乱改。」
